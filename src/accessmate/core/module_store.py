...
compares it against what is installed under %APPDATA%/AccessMate/modules/, and
installs / removes modules on request.  This is deliberately a *curated* store:
the index only ever points at AccessMate's own, controlled downloads – it must
never load third-party code from arbitrary URLs.

Reuses the download/version helpers from :mod:`accessmate.core.updater` and the
install location from :mod:`accessmate.core.module_loader`.
"""
...
from accessmate.core.module_loader import MODULES_DIR
...
def _local_index() -> list[StoreModule] | None:
    ...
    accessmate` from the repo shows and installs modules without a network or a
...
def fetch_index() -> list[StoreModule] | None:
    ...
    try:
        req = urllib.request.Request(
            INDEX_URL, headers={"User-Agent": "AccessMate"})
        with urllib.request.urlopen(req, timeout=_TIMEOUT) as resp:
            data = json.load(resp)
        return _parse_index(data)
    except Exception:
        log.info("store index fetch failed (offline?)", exc_info=True)
        return None
...
def install(module: StoreModule) -> None:
    ...
    %APPDATA%/AccessMate/modules/<id>/.  A restart picks it up.
    ...
    req = urllib.request.Request(
        module.download_url, headers={"User-Agent": "AccessMate"})
...
def _locate_module_folder(root: Path, subdir: str) -> Path | None:
    ...
    (``AccessMate-main/``), so we glob one level down.  ``subdir`` may be a
