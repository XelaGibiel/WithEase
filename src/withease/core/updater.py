...
GITHUB_REPO = "XelaGibiel/WithEase"
# List endpoint (not /releases/latest): the latter hides pre-releases, and
# WithEase ships pre-releases during the alpha, so we take the newest
...
def _update_via_zipball(info: ReleaseInfo) -> None:
    ...
    pkg_dir = Path(__file__).resolve().parents[1]   # …/withease
    with tempfile.TemporaryDirectory() as tmp:
        with zipfile.ZipFile(io.BytesIO(payload)) as zf:
            zf.extractall(tmp)
        # The zipball contains one top-level folder with the repo contents.
        candidates = list(Path(tmp).glob("*/src/withease"))
        if not candidates:
...
def restart_app() -> None:
    """Relaunch WithEase (reopening the settings window) and quit this one.
    ...
    if getattr(sys, "frozen", False):
        ...
        target = f'"{sys.executable}" -m withease --open-settings'
    subprocess.Popen(
